...
```{r 405, fig.cap='Evolution de la lisibilité moyenne des tweets de Trump', out.width='80%', fig.asp=.75, fig.align='center'}
foo<-foo %>%filter(nb_mots>1)
#la fonction de calcul de diversité
t1=Sys.time()
lexdiv<-tokens(foo$text)%>%
  textstat_lexdiv(foo$text, measure = c("CTTR", "Maas","MATTR"),remove_numbers = TRUE,
  remove_punct = TRUE,
  remove_symbols = TRUE,
  remove_hyphens = TRUE) 
t2=Sys.time()
...
Ils sont devenu extrêmement populaire même si l'effet esthétique est plus important de leur utilité analytique.
...
toks<- tokens(foo$text) %>% 
  ...
  dfm_remove(c("couleur", "to"))

docvars(toks,"influenceur")<-df$influenceur
...
ggsave("g0.jpg", plot=last_plot(), width = 27, height = 19, units = "cm")


foo<-toks %>% 
  dfm_group(groups = influenceur) %>%
    dfm_trim(min_termfreq = 5, verbose = FALSE)


freq <- textstat_frequency(foo, group =influenceur)
...
ggsave("g1.jpg", plot=last_plot(), width = 27, height = 19, units = "cm")
...
```{r 405, fig.cap=' ', out.width='80%', fig.asp=.75, fig.align='center'}
```


## Le cas de sources multiples

le cas dy corpus de Trump est passionnant car il donne toute la production d'un locuteur au cours du temps. Le plus souvent les corpus sont constitués de locuteurs multiples dont les niveaux d'engagement sont très variables, un petit nombre publie souvent, et la majorité de manière épisodique.

Il est important de quantifier ces rapports de production. 

### Mesure de la concentration

Qui produit combien? c'est la première question quand on analyse des données sociales. Les vieilles courbes de Lorenz sont à ce titre précieuses, tout autant que les indices de concentration, celui de Gini pour prendre un grand classsique. Le lecteur, avec un peu de curiosité, en trouvera facilement d'autres plus modernes ou mieux adaptés à nos besoins.

Illustrons.

```{r 406, fig.cap=' ', out.width='80%', fig.asp=.75, fig.align='center'}
```

### ou par segmentation

Il est souvent plus simple de représenter des catégories ordonnée plutôt qu'un gradient : les grous, les moyens et les petits producteurs. 


```{r 407, fig.cap=' ', out.width='80%', fig.asp=.75, fig.align='center'}
```
...
Le texte est une matière qui a un poids (le nombre de mot), une variétés (le nombre d'expression), une complexité (les règle qui l'organisent). nous veons de nous doter des premiers outils d'analyse, il est temps de passer à la suite. 
